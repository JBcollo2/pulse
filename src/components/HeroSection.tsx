      ...
      <div className="relative overflow-hidden w-full h-screen min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-green-900">
        ...
        <div className="absolute inset-0">
          {slides.map((slide, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/20 to-transparent lg:from-black/50 lg:via-black/15 lg:to-transparent" 
                   style={{ width: '60%' }} />
            </div>
          ))}
        </div>
        ...
        <div className="absolute top-3 right-3 md:top-4 md:right-4 lg:top-5 lg:right-5 z-20 flex items-center gap-1.5">
          <button
            ...
            className="w-8 h-8 md:w-9 md:h-9 lg:w-10 lg:h-10 bg-black/40 backdrop-blur-md border border-white/40 text-white hover:bg-black/60 hover:border-white/60 rounded-full transition-all duration-200 flex items-center justify-center shadow-xl hover:shadow-2xl hover:scale-110"
        ...
        >
          {slides.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 backdrop-blur-sm border border-white/30 ${
                index === currentSlide 
                  ? 'bg-white shadow-lg w-8 md:w-10 lg:w-12' 
                  : 'bg-white/60 hover:bg-white/80 w-2 md:w-2.5 lg:w-3'
              }`}
        ...
        <div className="relative z-10 flex flex-col h-full w-full px-3 md:px-4 lg:px-5">
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4 md:mb-5">
              <div className="inline-flex items-center gap-1.5 bg-white/20 backdrop-blur-lg border border-white/30 rounded-full px-3 py-1.5 md:px-4 md:py-2 text-white text-xs md:text-sm font-medium shadow-lg">
                {isLoadingEvents ? (
                  <>
              ...
              {slides[currentSlide] && !isLoadingEvents && (
                <div className="inline-flex items-center gap-1.5 bg-green-500/25 backdrop-blur-lg border border-green-400/40 rounded-full px-3 py-1.5 md:px-4 md:py-2 text-green-100 text-xs md:text-sm font-medium shadow-lg">
              ...
              {events.length === 0 && !isLoadingEvents && (
                <div className="inline-flex items-center gap-1.5 bg-yellow-500/25 backdrop-blur-lg border border-yellow-400/40 rounded-full px-3 py-1.5 md:px-4 md:py-2 text-yellow-100 text-xs md:text-sm font-medium shadow-lg">
                  Preview Mode
                </div>
              )}
            </div>
            ...
            <p 
              className="text-white/95 text-sm sm:text-base md:text-lg lg:text-xl mb-5 md:mb-6 max-w-2xl leading-relaxed font-light"
              style={{
            ...
            <div className="mb-6 md:mb-8 w-full max-w-lg">
              <div className="relative">
                <input
                  ...
                  className="w-full pl-9 md:pl-10 pr-16 md:pr-18 py-1.5 md:py-2 rounded-lg md:rounded-xl bg-white/20 backdrop-blur-lg border-2 border-white/30 text-white placeholder-white/70 focus:bg-white/25 focus:border-white/60 focus:ring-2 focus:ring-white/30 transition-all duration-300 text-xs md:text-sm focus:outline-none shadow-lg"
                />
                <Search className="absolute left-2.5 md:left-3 top-1/2 transform -translate-y-1/2 text-white/80 h-3.5 w-3.5 md:h-4 md:w-4" />
                <button
