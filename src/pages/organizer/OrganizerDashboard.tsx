...
interface Event {
  ...
  date: string;
  location: string;
  description?: string; // Add description to the Event interface if it's part of event creation
}
  ...
  const handleLogout = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/auth/logout`, {
        method: 'POST',
        credentials: 'include' // This ensures cookies are sent with the request to clear the session
      });
                    ...
                    organizerEvents.map(event => (
                      <Card key={event.id} className="flex flex-col h-full">
                        ...
                        <div className="p-4 border-t flex gap-2 justify-end">
                          {/* Add functionality for editing event */}
