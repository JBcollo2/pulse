...
const SystemReports = () => {
  ...
  // Fetch reports based on selected organizer and date range
  const fetchReports = async () => {
    setIsLoading(true);
    try {
      // Build URL with optional organizer_id and date range query parameters
      let url = `${import.meta.env.VITE_API_URL}/admin/reports/summary`;
  ...
  useEffect(() => {
    fetchReports();
  }, [selectedOrganizer, startDate, endDate, toast]); // Re-fetch when organizer selection or date range changes
  ...
  return (
    <div className="space-y-6">
      {/* Header with Organizer Filter and Date Range Picker */}
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
